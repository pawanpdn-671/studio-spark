	...
	const onChangePermission = async (subAccountId: string, val: boolean, permissionsId: string | undefined) => {
		...
		if (type === "agency") {
			await saveActivityLogsNotification({
				...
				subaccountId: subAccountPermissions?.Permissions.find((p) => p.subAccountId === subAccountId)?.SubAccount
					.id,
			});
		}
						...
						{authUserData?.role === "AGENCY_OWNER" && (
							<div>
								...
								<FormDescription className="mb-4">
									You can give SubAccount access to team member by turning on access control for each
									SubAccount. This is only visible to agency owners.
								</FormDescription>
								<div className="flex flex-col gap-4">
									{subAccounts?.map((subacc) => {
										const subAccountPermissionsDetails = subAccountPermissions?.Permissions.find(
											(per) => per.subAccountId === subacc.id,
										);
											...
											<div
												...
												<Switch
													...
													onCheckedChange={(permission) => {
														onChangePermission(subacc.id, permission, subAccountPermissionsDetails?.id);
													}}
												/>
											</div>
										);
