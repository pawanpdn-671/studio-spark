...
const Billing = async ({ params }: Props) => {
	...
	return (
		<>
          ...
          further question contact support@studio-spark-app.com"
				...
				{addOns.data?.map((addOn) => (
					<PricingCard
						...
						amt={
							//@ts-ignore
							addOn.default_price?.unit_amount ? `$${addOn.default_price.unit_amount / 100}` : "$0"
						}
						buttonCta="Subscribe"
						...
						highlightDescription="Get priority support and skip the long with the click of a button"
					/>
				))}
			</div>
			<h2 className="text-2xl p-4">Payment History</h2>
			<Table className="font-medium truncate">
